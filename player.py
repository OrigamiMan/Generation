...
class Player:
    def __init__(self) -> None:
        ...
        self.body_sprites = [pygame.image.load("./assets/character/walkcycle/BODY_male.png"),
                             pygame.image.load("./assets/character/walkcycle/TORSO_plate_armor_torso.png"),
                             pygame.image.load("./assets/character/walkcycle/LEGS_plate_armor_pants.png"),
                            pygame.image.load("./assets/character/walkcycle/FEET_plate_armor_shoes.png")]
        self.direction = 2
        self.stage = 0
        ...
        while open_set:            
            ...
            for n in neighbors:
                ...
                if n[0] != pos[0] and n[1] != pos[1]:
                    cost = g_score[pos] + dist(n, pos) * 0.8 
                else: 
                    cost = g_score[pos] + dist(n, pos)

                if n not in g_score or cost < g_score[n]:
                    g_score[n] = cost
                    h_score[n] = dist(n, self.walking_target)
                    came_from[n] = pos                
        
        rev_route = [tuple(self.walking_target)]
        while (last_pos := rev_route[-1]) != self.pos:
    ...
    def draw(self, surface: pygame.Surface, map_offset: pygame.Vector2) -> None:
        ...
        player_pos = 32*(self.pos - map_offset)
        for item in self.body_sprites:
            img = item.subsurface(crop_rect)
